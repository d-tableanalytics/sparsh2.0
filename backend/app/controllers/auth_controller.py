...
def has_hrms_access(user: dict) -> bool:
    ...
    Unlike Task/Delegation, the `tasks.access_task_management` override does NOT grant HRMS:
    a client company user must never reach employee/payroll/KYC data, even if an admin granted
    them task access. So we exclude client-side users explicitly before the internal check."""
