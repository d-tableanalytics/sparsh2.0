...
# The product runs on Indian Standard Time, and the recurrence engine decides every
# "which day is this" question there (see recurring_task_service), so the "has this task's day
# arrived yet" test in _task_totals is made on the same clock.
...
def _is_irm_delegated(doc: dict) -> bool:
    """IRM's Task-vs-Delegation split — NOT the same test as report_service.is_delegated
    (that one only asks who assigned the task, and also backs the unrelated Task Overview
    "delegators" stat, so it stays untouched).

    A recurring task is a "checklist" in this system: it is generated by the recurring
    engine (app.services.recurring_task_service) and re-scores itself every period via its
    own check points (task_credit), so it belongs under Task however it was assigned.
    Only a ONE-TIME task hand-assigned to someone else counts as Delegation. A self-assigned
    task always counts as Task, recurring or not.
    """
    if doc.get("assigned_to") != "other" or not doc.get("target_staff_id"):
        return False
    repeat = str(doc.get("repeat") or "").strip()
    return repeat in ("", "Does not repeat")
...
async def _task_totals(company_id: str, period: str, people: Dict[str, dict]) -> Dict[str, dict]:
    ...
    A task counts for whoever DOES it (`doer_ids`), split by how it reached them: a
    one-time task someone else assigned → Delegation Score; a recurring task/checklist
    (assigned by anyone) or a self-assigned task → Task. See `_is_irm_delegated`.
    """
    start_iso, end_iso = period_bounds(period)
    tasks = await fetch_tasks(start_iso, end_iso)
    cid = str(company_id)
    # An occurrence belongs to its IST calendar day (the whole recurrence engine is decided
    # there), so "has this task's day arrived" is judged in IST too.
    ...
    for doc in tasks:
        ...
        if doc_cid and doc_cid != cid:
            continue
        # Work that is not owed yet cannot have been done yet, so counting it would be a
        # penalty for being given a plan. A recurring task's whole series is now written when
        # it is created (calendar_events._expand_task_series) so the schedule is visible up
        # front; without this, assigning a daily checklist running to month-end would drop the
        # person's Task score to near zero the instant it was assigned, and it would climb back
        # only as each future day arrived. The month still ends on the same total — every
        # occurrence counts once its own day comes round.
        if str(doc.get("start") or "")[:10] > today_ist:
            continue
        bucket = "delegation" if _is_irm_delegated(doc) else "task"
