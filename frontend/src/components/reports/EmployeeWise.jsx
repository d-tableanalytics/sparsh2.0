...
import { getDoers, exportReport } from '../../services/reportApi';
...
const EmployeeWise = ({ params, onOpenEmployee }) => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getDoers({ ...params, search, sort, order, skip: page * PAGE_SIZE, limit: PAGE_SIZE });
      setData(res);
  ...
  const doExport = async (format) => {
    setExportOpen(false);
    setExporting(format);
    try { await exportReport({ format, ...params }); }
    catch (e) { /* handled globally */ }
  ...
  const cols = [
    ['name', 'Employee'], ['id', 'Emp ID'], ['company', 'Company'], ['department', 'Dept'],
    ['assigned', 'Assigned'], ['completed', 'Completed'], ['pending', 'Pending'], ['overdue', 'Overdue'],
    ['completionRate', 'Completion %'], ['score', 'Avg Score'], ['lastActivity', 'Last Activity'],
    ['rating', 'Status'],
  ];
            ...
            <table className="w-full text-left min-w-[1000px]">
              <thead>
                <tr className="border-b border-[var(--border)]">
                  {cols.map(([key, label]) => (
                    <th key={key} onClick={() => key !== 'id' && key !== 'lastActivity' && handleSort(key)}
                      className={`px-4 py-3 text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest select-none ${key !== 'id' && key !== 'lastActivity' ? 'cursor-pointer hover:text-[var(--text-main)]' : ''}`}>
              ...
              <tbody>
                {items.map((e) => (
                  <tr key={e.id} onClick={() => onOpenEmployee(e.id)}
                    ...
                    <td className="px-4 py-3 text-[12px] font-bold text-[var(--text-muted)]">—</td>
                    <td className="px-4 py-3 text-[12px] font-bold text-[var(--text-muted)]">{e.company || '—'}</td>
                    ...
                    <td className="px-4 py-3 text-[12px] text-[var(--text-muted)]">—</td>
                    <td className="px-4 py-3">
