...
import {
    ...
    Download, ExternalLink, BookOpen, Zap, AlertTriangle, Circle, CheckCircle, Info, Sparkles, Brain, Eye
} from 'lucide-react';
...
const CompanyPortal = () => {
    ...
    const fetchSessionTasks = async (sid) => {
        ...
        try {
            ...
            // Sync template assessments if linked
            if (ev.session_template_id) {
                try {
                    ...
                    console.error("Knowledge Check Sync Error:", tErr);
                }
            }
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-black text-[var(--text-main)] italic uppercase tracking-tighter leading-none flex items-center gap-4">
                        Neural <span className="text-[var(--accent-indigo)]">Roadmap</span>
                                        ...
                                        <div className="p-4 flex-1 space-y-2 overflow-y-auto no-scrollbar max-h-[350px]">
                                            {getSession(selectedSessionId)?.template_assessments?.length > 0 ? (
                                                getSession(selectedSessionId).template_assessments.map((quiz, idx) => (
                                                    <div key={idx} className="flex items-center gap-4 p-4 bg-[var(--input-bg)] border border-transparent hover:border-purple-500/30 rounded-2xl transition-all group cursor-pointer" onClick={() => navigate(`/assessment/${selectedSessionId}/${idx}`)}>
                                        ...
                                        <div className="p-4 flex-1 space-y-2 overflow-y-auto no-scrollbar max-h-[350px]">
                                            {getSession(selectedSessionId)?.resources?.length > 0 ? (
                                                getSession(selectedSessionId).resources.map((r) => (
                                                    <div key={r.id} className="flex items-center gap-4 p-4 bg-[var(--input-bg)] border border-transparent hover:border-amber-500/30 rounded-2xl transition-all group cursor-pointer" onClick={() => navigate(`/sessions/${selectedSessionId}/resource/${r.id}`)}>
                                            ...
                                            {getSession(selectedSessionId)?.contents?.length > 0 ? (
                                                getSession(selectedSessionId).contents.map((c, i) => (
                                                    <a key={i} href={c.url} target="_blank" rel="noopener noreferrer" 
                                                       onClick={() => api.post(`/calendar/events/${selectedSessionId}/resources/${c.id || i}/view`).catch(e => {})}
                                                       className="flex items-center gap-4 p-4 bg-[var(--input-bg)] border border-transparent hover:border-blue-500/30 rounded-2xl transition-all group">
