...
/* ─────────────────────────────────────────────────────────────
   SMOPS ▸ HOD Activity — per-HOD activity dashboard, scoped to the
   SMOPS user's companies via the global company selector. Identical
   look-and-feel to the Admin panel's HOD View. All data is mock.
   ───────────────────────────────────────────────────────────── */
...
const HODS = [
  ...
  {
    name: 'Neha Gupta', id: 'EMP-311', company: 'Orbit Media', role: 'HR', email: 'neha@orbit.media', actionClosure: 54,
    scoring: [
      { month: 'Jul', activity: 'Cal Disc', completed: 1, total: 3, missed: 2, pending: 0, score: 40 },
    ],
    occ: [
      { date: '2026-07-05', month: 'Jul', activity: 'Cal Disc', status: 'Completed' },
      { date: '2026-07-12', month: 'Jul', activity: 'Cal Disc', status: 'Missed' },
    ],
    needs: ['Calendar discipline slipping — 2 missed this month.'],
    actions: [{ activity: 'Cal Disc', action: 'Schedule cadence', owner: 'P. Shah', emp: 'EMP-204', target: '2026-07-18', status: 'Overdue', clientDelay: 'Delayed', omDelay: 'Pending', followup: '—' }],
  },
  {
    name: 'Vikram Singh', id: 'EMP-402', company: 'Cobalt Bank', role: 'MD', email: 'vikram@cobalt.bank', actionClosure: 90,
    scoring: [
      { month: 'Jul', activity: 'ORM', completed: 1, total: 1, missed: 0, pending: 0, score: 91 },
    ],
    occ: [{ date: '2026-07-08', month: 'Jul', activity: 'ORM', status: 'Completed' }],
    needs: [],
    actions: [],
  },
];
...
const HodActivity = () => {
  ...
  const list = isAll ? HODS : HODS.filter((h) => h.company === company.name);
  const [hodId, setHodId] = useState(HODS[0].id);
  const hod = list.find((h) => h.id === hodId) || list[0];
  ...
  const figures = useMemo(() => {
    ...
    return { total, completed, missed, pending, completion: total ? Math.round((completed / total) * 100) : 0, openActions: hod.actions.length, actionClosure: hod.actionClosure };
  }, [hod]);

  return (
    <div className="space-y-5">
      <DashboardHero icon={UserCog} title="HOD Activity" highlight={hod ? hod.name : (isAll ? 'All Companies' : company.name)} subtitle="Monitor what client HODs are doing across your companies">
        <HeaderSelect value={company.name} onChange={pickCompany} options={companies.map((c) => c.name)} />
        {list.length > 0 && <HeaderSelect value={hod?.id} onChange={setHodId} options={list.map((h) => h.id)} />}
      ...
      {!hod ? (
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] shadow-sm py-16 text-center">
          ...
          <p className="text-[13px] font-bold mt-3">No HODs for {company.name}</p>
          <p className="text-[12px] text-[var(--text-muted)] mt-1">Switch company from the top bar to view HOD activity.</p>
        </div>
      ) : (
        <>
          {/* Identity bar */}
