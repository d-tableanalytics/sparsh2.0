...
import { Bot, User, AlertCircle, StopCircle } from 'lucide-react';
...
export default function MessageBubble({ message, onEdit }) {
  ...
  return (
    ...
    >
      <div className={`flex gap-2.5 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        >
          {isUser ? (
            ...
            <>
              {/* Render whatever partial content arrived before stop */}
              {message.content ? (
      ...
      {/* Copy / Edit actions — visible on hover for user messages */}
      {isUser && (
